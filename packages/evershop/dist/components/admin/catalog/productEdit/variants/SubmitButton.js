...
export function SubmitButton({ productId, createProductApi, addVariantItemApi, productFormContextDispatch, modal: { closeModal }, refresh }) {
    ...
    const createVariant = async () => {
        ...
        if (Object.keys(productFormErrors).length > 0) {
            ...
            formData.set('url_key', `${formData.get('url_key')}-${formData.get('sku')}`);
    ...
    return (React.createElement(Button, { title: "Create", variant: "primary", onAction: createVariant, isLoading: loading }));
}
