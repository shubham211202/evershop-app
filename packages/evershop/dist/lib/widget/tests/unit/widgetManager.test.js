...
jest.unstable_mockModule('fs', () => ({
    existsSync: jest.fn((path) => {
        if (path.includes('InvalidComponent.js') ||
...
describe('Widget Manager Module', () => {
    ...
    describe('Mutation after getAllWidgets()', () => {
        ...
        it('should throw an error if updateWidget is called after getAllWidgets', async () => {
            ...
            widgetModule.getAllWidgets(); // This freezes the manager
            expect(() => widgetModule.updateWidget('ExistingWidget', { description: 'Updated' })).toThrow('Widget manager is in a read-only state. No further mutations are allowed. We suggest to register, update, or remove a widget from the bootstrap file.');
        });
        ...
        it('should throw error if trying to updateWidget with non-existing widget', async () => {
            const widgetModule = await import('../../widgetManager.js');
            expect(() => widgetModule.updateWidget('NonExistingWidget', { description: 'Test' })).toThrow('Widget not found');
        });
        it('should thrown an error if trying to update widget after calling getAllWidgets', async () => {
            ...
            widgetModule.getAllWidgets(); // This freezes the manager
            expect(() => widgetModule.updateWidget('WidgetToUpdate', { description: 'New Desc' })).toThrow('Widget manager is in a read-only state. No further mutations are allowed. We suggest to register, update, or remove a widget from the bootstrap file.');
        });
...
//# sourceMappingURL=widgetManager.test.js.map