        ...
        try {
            ...
            if (existsSync(mediaPath)) {
                ...
                await sharp(mediaPath)
                    .resize(getConfig('catalog.product.image.thumbnail.width', 100), getConfig('catalog.product.image.thumbnail.height', 100), { fit: 'inside' })
                    .toFile(path.join(CONSTANTS.MEDIAPATH, thumbnailPath));
                // Generate listing
                await sharp(mediaPath)
                    .resize(getConfig('catalog.product.image.listing.width', 250), getConfig('catalog.product.image.listing.height', 250), { fit: 'inside' })
                    .toFile(path.join(CONSTANTS.MEDIAPATH, listingPath));
                // Generate single
                await sharp(mediaPath)
                    .resize(getConfig('catalog.product.image.single.width', 500), getConfig('catalog.product.image.single.height', 500), { fit: 'inside' })
...
//# sourceMappingURL=localGenerateProductImageVariant.js.map