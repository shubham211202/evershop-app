            ...
            resolvers: [
                async function resolver(shippingAddressId) {
                    if (!shippingAddressId || !this.getData('shipping_zone_id')) {
                        ...
                        shippingZoneQuery.where('shipping_zone.country', '=', shippingAddress.country);
                        const shippingZoneProvinces = await shippingZoneQuery.execute(pool);
                        if (shippingZoneProvinces.length === 0) {
                            return null;
                        }
                        else {
                            const check = shippingZoneProvinces.find((p) => p.province === shippingAddress.province || p.province === null);
                            if (!check) {
                                return null;
                            }
                            else {
