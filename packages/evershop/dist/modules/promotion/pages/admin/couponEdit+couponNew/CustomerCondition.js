...
export default function CustomerCondition({ coupon = {}, groups: { items: customerGroups } }) {
    ...
    const selectedGroups = (condition.groups || [])
        .filter((g) => customerGroups.find((group) => parseInt(group.value, 10) === parseInt(g, 10)))
        .map((g) => {
        const group = customerGroups.find((e) => parseInt(e.value, 10) === parseInt(g, 10));
        return {
    ...
    return (React.createElement(Area, { id: "couponCustomerCondition", coreComponents: [
            {
                component: {
                    default: () => (React.createElement(Select, { name: "user_condition[groups][]", options: customerGroups.map((group) => ({
                            value: group.value.toString(),
                            label: group.name
            ...
            },
            {
                ...
                props: {
                    ...
                    suffix: (React.createElement("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", style: { width: '1.8rem', height: '1.8rem' }, viewBox: "0 0 24 24", strokeWidth: 1.5, stroke: "currentColor", className: "w-6 h-6" },
