    ...
    return [
        ...
        async function discountToSpecificProductsCalculator(cart, coupon) {
            ...
            const collections = await select()
                ...
                .where('product_id', 'IN', items.map((item) => item.getData('product_id')))
                .execute(pool);
            items.forEach((item) => {
                ...
                targetProducts.forEach((targetProduct) => {
                    ...
                    if (key === 'collection') {
                        ...
                        if (operator === 'IN') {
                            flag = collections.some((collection) => requiredCollectionIDs.includes(collection.collection_id) &&
                ...
                if (coupon.discount_type === 'fixed_discount_to_specific_products') {
                    discountAmount = Math.min(discountAmount, priceIncludingTax
                    ...
                    discounts[item.getId()] = toPrice(((discountPercent *
                        (priceIncludingTax
                            ? item.getData('final_price_incl_tax')
                            : item.getData('final_price'))) /
                        100) *
        ...
        async function buyXGetYCalculator(cart, coupon) {
            ...
            configs.forEach((row) => {
                var _a;
                const sku = (_a = row.sku) !== null && _a !== void 0 ? _a : null;
                ...
                for (let i = 0; i < items.length; i += 1) {
                    const item = items[i];
                    if (item.getData('product_sku') === sku.trim() &&
                        item.getData('qty') >= buyQty + getQty) {
                        const discountPerUnit = toPrice((discount *
                            (priceIncludingTax
                                ? item.getData('final_price_incl_tax')
                                : item.getData('final_price'))) /
                            100);
...
//# sourceMappingURL=registerDefaultCalculators.js.map